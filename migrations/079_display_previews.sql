-- Stop old media workers before applying; source objects are never changed.
-- Rebuild only completed legacy derivatives, leaving active and failed jobs alone.
WITH candidates AS (
  SELECT media.asset_id
  FROM asset_media AS media
  ...
  WHERE asset.status = 'ready' AND asset.archived_at IS NULL
    ...
    AND asset.kind IN ('视频', '图片', '音频')
    AND media.status = 'ready'
    AND COALESCE(media.review_proxy_object_key, '') NOT LIKE '%/preview-v2.%'
